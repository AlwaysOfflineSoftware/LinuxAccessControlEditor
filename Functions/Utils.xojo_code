...
Protected Module Utils
	...
	#tag Method, Flags = &h0
		Function ReadBinData(path As String) As memoryblock
		  If(Utils.ValidatePath(path)) Then
		    Var filePath As FolderItem= New FolderItem(path)
		    Var Binstream As BinaryStream= BinaryStream.Open(filePath, False)
		    Var memblock As MemoryBlock= Binstream.Read(Binstream.Length)
		    Binstream.Close
		    Return memblock
		  End
	...
	#tag Method, Flags = &h0
		Sub WriteBinData(path as folderItem, data as memoryBlock, overwrite as boolean)
		  Var fullFilePath As FolderItem= path
		  Var output As TextOutputStream
		  
		  Try
		    Var Binstream As BinaryStream = BinaryStream.Create(fullFilePath, overwrite)
	...
	#tag Method, Flags = &h0
		Sub WriteBinData(path as string, data as memoryBlock, overwrite as boolean)
