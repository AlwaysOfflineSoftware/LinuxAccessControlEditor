#tag DesktopWindow
Begin DesktopWindow PasswordInputScreen
   Backdrop        =   0
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  Self.txt_FileToDecrypt.Text= MainScreen.txt_FileSelected.Text
	...
	#tag Event
		Sub Pressed()
		  ...
		  If(Self.chk_UseSameDir.Value) Then
		    PasswordHandler.EncryptedUnzip(targetPath, Self.txt_TestPassword.Text)
		  Else
		    landingPath= New FolderItem(Self.txt_DecryptTo.Text)
		    PasswordHandler.EncryptedUnzip(_
		    targetPath, Self.txt_TestPassword.Text, landingPath)
		  End 
		  
		  Utils.GeneratePopup(1,"File was Decrypted!", "")
		End Sub
	#tag EndEvent
#tag EndEvents
